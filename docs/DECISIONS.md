...
| D25 | **Play** is a keep-alive tab; on other tabs a **bottom-right mini player** shows the same session (not `display:none`). Card **Watch now** plays in the mini player without changing tab; **Shift-click** opens the Play tab | Interrupt-free browsing; shortcut for full player |
| D26 | **Hard split Discovery vs Sorted:** Home / Search / Channel are **Discovery** and must never show now-playing or up-next (live exclude + client filter). **Queue** is **Sorted** (“wanna watch”). Card triage: **Watch now** (mini player, stay on tab); **Watch later** removes from Discovery until dequeued; **Not watching** (mark-watched / hide) also removes from Discovery. Watch plays now (current stays at front of up-next; up-next never cleared); drag/context reorder; on ended → mark watched + autoplay next | Feed sorts into watch-later vs not-watching; never mix Sorted back into Discovery |
