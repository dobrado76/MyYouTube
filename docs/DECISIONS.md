...
| D26 | **Queue** tab + card actions: **Watch** plays now (current item stays at front of up-next; up-next is never cleared); **Queue** appends; drag/context reorder; on ended → mark watched + autoplay next | Intentional playlist without YouTube Home autoplay-next |
...
| D29 | Player overlay **Previous / Next** (Play + mini player) walks `playHistory` / up-next; **Next** with progress **> 60%** marks watched and drops the item from the session (no playHistory); below that, Previous still works; ended always marks watched | Queue navigation without leaving the player |
